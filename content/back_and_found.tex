...
As a result, the input space is effectively infinite.
Our pool is then created by sampling a finite set of IC and PDE parameter pairs.
...
If we created a pool we can evaluate the uncertainty for each sample inside. 
...
We query samples to our oracle directly from our input space or create new samples we then query~\cite{wang2015active}. 
Querying directly from the inputspace can be a great advantage for us. 
Previously the uncertainty was mentioned, and how sampling from higher uncertainties can 
reduce the amount of training samples. From given uncertainty we can make use of the possibility 
of calculating the gradient of it. This allowes us to sample from regions with the steepest ascent, 
even for a global perspective. The challenge here is to choose a fitting algorithm, defining the 
query strategy, this is where a chose an implementation of Markov chain Monte Carlo.
