...
Usually, the amount of training data needs to be quite large to keep the deviation between the expected output of our model and the actual output small. 
...
or text categorisation ~\cite{lewis1995sequential}. 
Neural PDE solvers are particularly affected by the high cost of labelling.

Typically, the solution to a PDE is approximated using a numerical solver, which is both resource-intensive and time-consuming. 
...
Therefore, pool-based active learning was introduced 
by Musekamp et al. ~\cite{musekamp2024active} and applied to different neural surrogate models in the context of PDEs.
...
to replace the old pool-based method with a query synthesis strategy using the gradient, 
